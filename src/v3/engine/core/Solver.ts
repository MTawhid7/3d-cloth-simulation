...
import { Topology } from '../utils/Topology'; // Import Topology
...
export class Solver {
    ...
    constructor(proxyMesh: THREE.Mesh, collisionMesh: THREE.Mesh) {
        ...
        // --- NEW: Detect Collar and Apply ---
        const collarIndices = Topology.getNeckIndices(proxyMesh.geometry, this.data.positions);
        this.distanceConstraints.setCollar(collarIndices);
    }
    ...
    public update(dt: number) {
        ...
        for (let step = 0; step < substeps; step++) {
            ...
            // 1. Soft Constraints (XPBD)
            this.distanceConstraints.solve(this.data, sdt);
            this.bendingConstraints.solve(this.data, sdt);

            // 2. Hard Limits (The Fix for Stretching)
            this.distanceConstraints.solveHardLimits(this.data);
    ...
    private solveCollisions() {
        for (let i = 0; i < this.data.count; i++) {
            ...
            if (result.collided) {
                ...
                if (result.rescued) {
                    // Kill velocity completely on rescue
                    this.data.prevPositions[idx] = this.data.positions[idx];
                    this.data.prevPositions[idx + 1] = this.data.positions[idx + 1];
                    this.data.prevPositions[idx + 2] = this.data.positions[idx + 2];
                } else {
                    // --- NEW: Substep Damping ---
                    // Aggressively dampen velocity for particles in contact
                    // This stops the "micro-bouncing" vibration
                    const damping = 0.8; // Retain only 80% of velocity per substep

                    this.data.prevPositions[idx] += (this.data.positions[idx] - this.data.prevPositions[idx]) * damping;
                    this.data.prevPositions[idx + 1] += (this.data.positions[idx + 1] - this.data.prevPositions[idx + 1]) * damping;
                    this.data.prevPositions[idx + 2] += (this.data.positions[idx + 2] - this.data.prevPositions[idx + 2]) * damping;
                }
            }
        }
    }
